...
async fn list_users(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<UserProfile> = stmt
        .query_map([], |row| {
            Ok(UserProfile {
                id: row.get(0)?,
                username: row.get(1)?,
...
async fn create_user(
    ...
    if form.username.trim().is_empty() || form.full_name.trim().is_empty() {
        return (
            ...
            Json(json!({ "success": false, "error": "Username and full name are required." })),
        );
    }
    ...
    match result {
        Ok(_) => (
            StatusCode::CREATED,
            Json(
                json!({ "success": true, "data": { "id": db.last_insert_rowid(), "username": form.username } }),
            ),
        ),
        Err(e) => {
...
async fn update_user(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "User updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "User not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update user." })),
            )
        }
    }
}
...
async fn delete_user(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "User deactivated." } })),
        ),
        Ok(_) => (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "Cannot delete admin user." })),
        ),
        Err(e) => {
            tracing::error!("Failed to delete user: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete user." })),
            )
        }
    }
}
...
async fn reset_password(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Password reset." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "User not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to reset password." })),
            )
        }
    }
}
...
async fn toggle_user_status(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "User status toggled." } })),
        ),
        Ok(_) => (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "Cannot toggle admin status." })),
        ),
        Err(e) => {
            tracing::error!("Failed to toggle status: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to toggle status." })),
            )
        }
    }
}
...
async fn get_role(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Role not found." })),
        ),
    }
}
...
async fn create_role(
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create role." })),
            )
        }
    }
}
...
async fn update_role(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Role updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Role not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update role." })),
            )
        }
    }
}
...
async fn delete_role(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    if is_system {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "Cannot delete system role." })),
        );
    }
    db.execute("DELETE FROM role_permissions WHERE role_id = ?1", [id])
        .ok();
    let result = db.execute("DELETE FROM roles WHERE id = ?1", [id]);
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Role deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Role not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete role." })),
            )
        }
    }
}
...
async fn list_all_permissions(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<Permission> = stmt
        .query_map([], |row| {
            Ok(Permission {
                id: row.get(0)?,
                permission_name: row.get(1)?,
                module: row.get(2)?,
                action: row.get(3)?,
...
async fn update_role_permissions(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<RolePermissionUpdate>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    db.execute("DELETE FROM role_permissions WHERE role_id = ?1", [id])
        .ok();
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": { "message": "Permissions updated." } })),
    )
}
...
async fn update_settings(
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": { "message": "Settings updated." } })),
    )
}
...
async fn list_integrations(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": integrations })),
    )
}
...
async fn update_integration(
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": format!("Integration '{}' updated.", service) } }),
        ),
    )
}
