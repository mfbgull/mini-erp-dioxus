    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                ...
                Json(json!({ "success": false, "error": "Failed to save draft." })),
            )
        }
    }
}
...
async fn get_draft(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            ...
            Json(json!({ "success": false, "error": "Draft not found." })),
        ),
    }
}
...
async fn update_draft(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Draft updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Draft not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to update draft: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update draft." })),
            )
        }
    }
}
...
async fn delete_draft(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Draft deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Draft not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete draft." })),
            )
        }
    }
}
...
async fn list_tax_rates(State(_state): State<AppState>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let mut stmt = db
        .prepare("SELECT id, name, rate FROM tax_rates WHERE is_active = 1")
        .unwrap();
    let items: Vec<serde_json::Value> = stmt.query_map([], |row| {
...
async fn list_payment_terms(State(_state): State<AppState>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let mut stmt = db
        .prepare("SELECT id, name, days FROM payment_terms WHERE is_active = 1")
        .unwrap();
    let items: Vec<serde_json::Value> = stmt.query_map([], |row| {
...
async fn submit_from_mobile(
    ...
    if items.is_empty() {
        return (
            ...
            Json(json!({ "success": false, "error": "No items provided." })),
        );
    }
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(
                    json!({ "success": true, "data": { "id": inv_id, "invoice_no": invoice_no } }),
                ),
            )
        }
        Err(e) => {
            tracing::error!("Failed to submit mobile invoice: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to submit invoice." })),
            )
        }
    }
}
