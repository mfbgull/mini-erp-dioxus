...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/invoices/recalculate-totals",
            post(recalculate_invoice_totals),
        )
}
    ...
    let items: Vec<Invoice> = stmt
        .query_map([], |row| {
            Ok(Invoice {
                ...
                returned_amount: row.get(14)?,
                discount_scope: row.get(15)?,
                discount_type: row.get(16)?,
                discount_value: row.get(17)?,
                tax_rate: row.get(18)?,
                notes: row.get(19)?,
                warehouse_id: row.get(20)?,
...
async fn get_invoice(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(inv) => {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "invoice": inv, "items": items } })),
            )
        }
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Invoice not found." })),
        ),
    }
}
        ...
        Ok(_) => {
            ...
            if !txn_ok {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to create invoice (transaction rolled back)." }),
                    ),
                );
            }
            ...
            if let Err(e) = db.execute_batch("COMMIT") {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to commit invoice (transaction rolled back)." }),
                    ),
                );
            }
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": inv })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create invoice." })),
            )
        }
    }
}
...
async fn update_invoice(
    ...
    let old_total: f64 = db
        .query_row(
            "SELECT COALESCE(total_amount, 0) FROM invoices WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .unwrap_or(0.0);
    let old_customer_id: i64 = db
        .query_row(
            "SELECT COALESCE(customer_id, 0) FROM invoices WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .unwrap_or(0);
    ...
    let result = db.execute(
        ...
        rusqlite::params![
            form.customer_id,
            form.source_type.as_deref().unwrap_or("DIRECT"),
            form.invoice_date,
            form.due_date,
            total_amount,
            form.discount_scope,
    ...
    match result {
        Ok(rows) if rows > 0 => {
            ...
            if let Err(e) = db.execute(
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to update invoice (transaction rolled back)." }),
                    ),
                );
            }
            ...
            {
                let mut old_stmt = db
                    .prepare("SELECT item_id, quantity FROM invoice_items WHERE invoice_id = ?1")
                    .unwrap();
            ...
            if let Err(e) = db.execute_batch("COMMIT") {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to commit invoice update (transaction rolled back)." }),
                    ),
                );
            }
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Invoice updated." } })),
            )
        }
        Ok(_) => {
            let _ = db.execute_batch("ROLLBACK");
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update invoice." })),
            )
        }
    }
}
...
async fn cancel_invoice(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    let invoice_exists: bool = db
        .query_row(
            "SELECT COUNT(*) FROM invoices WHERE id = ?1 AND status != 'Cancelled'",
            [id],
            |row| row.get::<_, i64>(0),
        )
        .unwrap_or(0)
    ...
    if !invoice_exists {
        ...
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "Invoice not found or already cancelled." })),
        );
    }
    ...
    let total_amount: f64 = db
        ...
        .unwrap_or(0.0);
    let returned_amount: f64 = db
        .query_row(
            "SELECT returned_amount FROM invoices WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .unwrap_or(0.0);
    ...
    let warehouse_id: i64 = db
        .query_row(
            "SELECT COALESCE(warehouse_id, 1) FROM invoices WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .unwrap_or(1);
    ...
    if net_amount > 0.0 {
        ...
        if let Err(e) = db.execute(
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(
                    json!({ "success": false, "error": "Failed to cancel invoice (transaction rolled back)." }),
                ),
            );
        }
    }
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": { "message": "Invoice cancelled and reversed." } })),
    )
}
...
async fn return_invoice(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(req): Json<InvoiceReturnRequest>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    if let Err(e) = db.execute(
        "UPDATE invoices SET returned_amount = ?1, updated_at = datetime('now') WHERE id = ?2",
        rusqlite::params![total_returned, id],
    ) {
        let _ = db.execute_batch("ROLLBACK");
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "success": false, "error": "Failed to record return (transaction rolled back)." }),
            ),
        );
    }
    ...
    if let Err(e) = db.execute_batch("COMMIT") {
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "success": false, "error": "Failed to commit return (transaction rolled back)." }),
            ),
        );
    }
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": "Return recorded.", "returned_amount": total_returned } }),
        ),
    )
}
...
async fn invoice_payments(
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": payments })),
    )
}
...
async fn recalculate_invoice_totals(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    if let Err(e) = db.execute_batch("COMMIT") {
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({ "success": false, "error": "Failed to commit recalculation." })),
        );
    }
