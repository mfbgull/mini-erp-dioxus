...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/forecasts/config",
            get(list_model_configs).post(create_model_config),
        )
        .route(
            "/api/forecasts/config/{id}",
            get(get_model_config)
                .put(update_model_config)
                .delete(delete_model_config),
        )
        .route(
            "/api/forecasts/seasonal-events",
            get(list_seasonal_events).post(create_seasonal_event),
        )
        .route(
            "/api/forecasts/seasonal-events/{id}",
            put(update_seasonal_event).delete(delete_seasonal_event),
        )
        .route(
            "/api/forecasts/demand-timeline",
            get(report_demand_timeline),
        )
}
...
async fn list_forecasts(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<DemandForecast> = stmt
        .query_map([], |row| {
            Ok(DemandForecast {
                id: row.get(0)?,
                item_id: row.get(1)?,
                item_name: row.get(2)?,
                item_code: row.get(3)?,
                forecast_date: row.get(4)?,
                period: row.get(5)?,
                predicted_quantity: row.get(6)?,
...
async fn run_forecast(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "run_id": run_id, "items_processed": items_count, "status": "completed" } }),
        ),
    )
}
...
async fn list_forecast_runs(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<ForecastRun> = stmt
        .query_map([], |row| {
            Ok(ForecastRun {
                id: row.get(0)?,
                run_id: row.get(1)?,
...
async fn list_forecast_accuracy(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<ForecastAccuracy> = stmt
        .query_map([], |row| {
            Ok(ForecastAccuracy {
                id: row.get(0)?,
...
async fn get_model_config(
    ...
    match result {
        ...
        Err(_) => (
            ...
            Json(json!({ "success": false, "error": "Config not found." })),
        ),
    }
}
...
async fn create_model_config(
    State(_state): State<AppState>,
    Json(form): Json<ForecastModelConfigForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                ...
                Json(json!({ "success": false, "error": "Failed to create config." })),
            )
        }
    }
}
...
async fn update_model_config(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Config updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Config not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update config." })),
            )
        }
    }
}
...
async fn delete_model_config(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Config deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Config not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete config." })),
            )
        }
    }
}
...
async fn list_seasonal_events(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<SeasonalEvent> = stmt
        .query_map([], |row| {
            Ok(SeasonalEvent {
                id: row.get(0)?,
                ...
                applies_to_category: row.get(5)?,
                applies_to_item_id: row.get(6)?,
            })
        })
        .unwrap()
        .filter_map(|r| r.ok())
...
async fn create_seasonal_event(
    State(_state): State<AppState>,
    Json(form): Json<SeasonalEventForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create event." })),
            )
        }
    }
}

async fn update_seasonal_event(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<SeasonalEventForm>,
) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Event updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update event." })),
            )
        }
    }
}

async fn delete_seasonal_event(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Event deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Event not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete event." })),
            )
        }
    }
}
...
async fn report_demand_timeline(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    // Build timeline
    let mut periods: std::collections::BTreeMap<String, serde_json::Value> =
        std::collections::BTreeMap::new();
    for row in &historical {
        let ym = row["ym"].as_str().unwrap_or("").to_string();
        let qty = row["qty"].as_f64().unwrap_or(0.0);
        let month_num = if ym.len() >= 7 {
            ym[5..7].parse::<usize>().unwrap_or(1)
        } else {
            1
        };
    ...
    for row in &forecasted {
        ...
        if let Some(entry) = periods.get_mut(&period) {
            ...
            let month_num = if period.len() >= 7 {
                period[5..7].parse::<usize>().unwrap_or(1)
            } else {
                1
            };
