...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/accounting/accounts/{code}/balance",
            get(account_balance),
        )
        ...
        .route(
            "/api/employees/{id}/salary-payments",
            get(list_salary_payments),
        )
        // Journal Entries
        .route(
...
async fn list_accounts(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<ChartOfAccount> = stmt
        .query_map([], |row| {
            Ok(ChartOfAccount {
                id: row.get(0)?,
                code: row.get(1)?,
...
async fn account_balances(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<AccountBalance> = stmt
        .query_map([], |row| {
            Ok(AccountBalance {
                id: row.get(0)?,
                code: row.get(1)?,
...
async fn get_account(
    ...
    match result {
        ...
        Err(_) => (
            ...
            Json(json!({ "success": false, "error": "Account not found." })),
        ),
    }
}
...
async fn list_periods(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<AccountingPeriod> = stmt
        .query_map([], |row| {
            Ok(AccountingPeriod {
                id: row.get(0)?,
...
async fn get_period(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Period not found." })),
        ),
    }
}
...
async fn update_period(
    ...
    let result = db.execute(
        "UPDATE accounting_periods SET status = ?1 WHERE id = ?2",
        rusqlite::params![status, id],
    );
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Period updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Period not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to update period: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update period." })),
            )
        }
    }
}
...
async fn list_tax_rates(State(_state): State<AppState>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let mut stmt = db
        .prepare("SELECT id, name, rate, is_default, is_active FROM tax_rates WHERE is_active = 1")
        .unwrap();
    let items: Vec<TaxRate> = stmt
        .query_map([], |row| {
            Ok(TaxRate {
                id: row.get(0)?,
                name: row.get(1)?,
...
async fn list_payment_terms(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        .prepare(
            "SELECT id, name, days, is_default, is_active FROM payment_terms WHERE is_active = 1",
        )
        .unwrap();
    let items: Vec<PaymentTerm> = stmt
        .query_map([], |row| {
            Ok(PaymentTerm {
                id: row.get(0)?,
                name: row.get(1)?,
...
async fn list_expenses(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<Expense> = stmt
        .query_map([], |row| {
            Ok(Expense {
                id: row.get(0)?,
                expense_no: row.get(1)?,
                category: row.get(2)?,
...
async fn get_expense(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Expense not found." })),
        ),
    }
}
...
async fn create_expense(
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": expense_id, "expense_no": eno } })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create expense." })),
            )
        }
    }
}
...
async fn delete_expense(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => {
            if let Err(e) = db.execute_batch("COMMIT") {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to delete expense (transaction rolled back)." }),
                    ),
                );
            }
            (
                StatusCode::OK,
                Json(
                    json!({ "success": true, "data": { "message": "Expense deleted and GL entry reversed." } }),
                ),
            )
        }
        Ok(_) => {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete expense." })),
            )
        }
    }
}
...
async fn list_expense_categories(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<ExpenseCategory> = stmt
        .query_map([], |row| {
            Ok(ExpenseCategory {
                id: row.get(0)?,
...
async fn create_expense_category(
    State(_state): State<AppState>,
    Json(form): Json<ExpenseCategoryForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
        "INSERT INTO expense_categories (category_name) VALUES (?1)",
        [&form.category_name],
    );
    match result {
        Ok(_) => (
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create category." })),
            )
        }
    }
}
    ...
    let items: Vec<Employee> = stmt
        .query_map([], |row| {
            Ok(Employee {
                ...
                bank_account_no: row.get(14)?,
                emergency_contact_name: row.get(15)?,
                emergency_contact_phone: row.get(16)?,
...
async fn get_employee(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Employee not found." })),
        ),
    }
}
...
async fn create_employee(
    ...
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let seq: i64 = db
        .query_row("SELECT COUNT(*) + 1 FROM employees", [], |row| row.get(0))
        .unwrap_or(1);
    ...
    match result {
        Ok(_) => (
            StatusCode::CREATED,
            Json(
                json!({ "success": true, "data": { "id": db.last_insert_rowid(), "employee_code": ecode } }),
            ),
        ),
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create employee." })),
            )
        }
    }
}
...
async fn update_employee(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Employee updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Employee not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update employee." })),
            )
        }
    }
}
...
async fn delete_employee(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
        "UPDATE employees SET is_active = 0, updated_at = datetime('now') WHERE id = ?1",
        [id],
    );
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Employee deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Employee not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete employee." })),
            )
        }
    }
}
...
async fn pay_salary(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<SalaryPaymentForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    if already_paid {
        return (
            StatusCode::CONFLICT,
            Json(json!({ "success": false, "error": "Salary already paid for this month." })),
        );
    }
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(
                    json!({ "success": true, "data": { "id": sp_id, "message": "Salary payment recorded." } }),
                ),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to record salary: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to record salary payment." })),
            )
        }
    }
}

async fn list_salary_payments(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
...
async fn create_journal_entry(
    ...
    if form.lines.is_empty() {
        return (
            ...
            Json(json!({ "success": false, "error": "At least one journal line is required." })),
        );
    }
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": entry_id } })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create journal entry." })),
            )
        }
    }
}
    ...
    match entry {
        Ok(e) => {
            let mut stmt = db
                ...
                .unwrap();
            let lines: Vec<JournalLine> = stmt
                .query_map([id], |row| {
                    Ok(JournalLine {
                        id: row.get(0)?,
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "entry": e, "lines": lines } })),
            )
        }
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Journal entry not found." })),
        ),
    }
}
