...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/reports/customer-statements",
            get(report_customer_statements),
        )
        ...
        .route(
            "/api/reports/stock-valuation/fifo",
            get(report_stock_valuation_fifo),
        )
        .route(
            "/api/reports/stock-history/{itemId}",
            get(report_stock_history),
        )
        .route("/api/reports/profit-loss", get(report_profit_loss))
        .route(
            "/api/reports/profit-loss/by-item",
            get(report_profit_loss_by_item),
        )
        ...
        .route(
            "/api/reports/trend-decomposition",
            get(report_trend_decomposition),
        )
        .route("/api/reports/expenses", get(report_expenses))
        .route(
            "/api/reports/purchase-summary",
            get(report_purchase_summary),
        )
        .route(
            "/api/reports/supplier-analysis",
            get(report_supplier_analysis),
        )
        .route(
            "/api/reports/production-summary",
            get(report_production_summary),
        )
        .route(
            "/api/reports/inventory-movement",
            get(report_inventory_movement),
        )
        .route("/api/reports/bom-usage", get(report_bom_usage))
        .route(
            "/api/reports/batch-traceability/{itemId}",
            get(report_batch_traceability),
        )
        .route("/api/reports/test-fifo", post(test_fifo_scenario))
        // Custom Reports
        .route(
            "/api/reports/custom",
            get(list_custom_reports).post(create_custom_report),
        )
        .route(
            "/api/reports/custom/{id}",
            put(update_custom_report).delete(delete_custom_report),
        )
}
...
async fn report_dso(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let credit_sales: f64 = db
        ...
        .unwrap_or(0.0);
    let dso = if credit_sales > 0.0 {
        (ar / credit_sales) * 30.0
    } else {
        0.0
    };
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "dso": dso, "accounts_receivable": ar, "credit_sales": credit_sales } }),
        ),
    )
}
...
async fn report_stock_history(
    ...
    let movements_with_balance: Vec<serde_json::Value> = movements
        .into_iter()
        .map(|m| {
    ...
    // Summary stats
    let total_in: f64 = movements_with_balance
        .iter()
        .filter(|m| {
            matches!(
                m["movement_type"].as_str(),
                Some("IN") | Some("TRANSFER_IN")
            )
        })
        .map(|m| m["quantity"].as_f64().unwrap_or(0.0))
        .sum();
    let total_out: f64 = movements_with_balance
        .iter()
        .filter(|m| {
            matches!(
                m["movement_type"].as_str(),
                Some("OUT") | Some("TRANSFER_OUT")
            )
        })
        .map(|m| m["quantity"].as_f64().unwrap_or(0.0))
        .sum();
    let total_adjusted: f64 = movements_with_balance
        .iter()
...
async fn report_profit_loss_by_item(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let revenue_by_item = query_report!(
        db,
    ...
    let purchase_by_item = query_report!(
        db,
    ...
    let mut items: Vec<serde_json::Value> = revenue_by_item
        .into_iter()
        .map(|row| {
    ...
    let total_revenue: f64 = items
        .iter()
        .map(|i| i["revenue"].as_f64().unwrap_or(0.0))
        .sum();
    let total_cogs: f64 = items
        .iter()
        .map(|i| i["cogs"].as_f64().unwrap_or(0.0))
        .sum();
...
async fn report_cash_flow(State(_state): State<AppState>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let cash_in: f64 = db
        .query_row("SELECT COALESCE(SUM(amount), 0) FROM payments", [], |r| {
            r.get(0)
        })
        .unwrap_or(0.0);
    let cash_out: f64 = db
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "operating_in": cash_in, "operating_out": cash_out + expense_out, "net_cash_flow": cash_in - cash_out - expense_out } }),
        ),
    )
}
...
async fn report_tax_summary(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let sales_tax: Vec<serde_json::Value> = sales_tax
        .into_iter()
        .map(|mut row| {
...
async fn report_batch_traceability(
    State(_state): State<AppState>,
...
async fn list_custom_reports(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<CustomReport> = stmt
        .query_map([], |row| {
            Ok(CustomReport {
                id: row.get(0)?,
                user_id: row.get(1)?,
...
async fn create_custom_report(
    State(_state): State<AppState>,
    Json(form): Json<CustomReportForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        Ok(_) => (
            StatusCode::CREATED,
            Json(json!({ "success": true, "data": { "id": db.last_insert_rowid() } })),
        ),
        Err(e) => {
            tracing::error!("Failed to create report: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create report." })),
            )
        }
    }
}

async fn update_custom_report(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<CustomReportForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
        "UPDATE custom_reports SET name=?1, config=?2 WHERE id=?3",
        rusqlite::params![form.name, form.config, id],
    );
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Report updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Report not found." })),
        ),
        Err(e) => {
            tracing::error!("Failed to update report: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update report." })),
            )
        }
    }
}
...
async fn delete_custom_report(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Report deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Report not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete report." })),
            )
        }
    }
}
...
async fn test_fifo_scenario(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    db.execute(
        "UPDATE items SET current_stock = 0 WHERE id = ?1",
        [item_id],
    )
    .ok();
    db.execute("DELETE FROM stock_balances WHERE item_id = ?1", [item_id])
        .ok();
    db.execute("DELETE FROM stock_batches WHERE item_id = ?1", [item_id])
        .ok();
    db.execute(
        "DELETE FROM purchases WHERE item_id = ?1 AND warehouse_id = ?2",
    ...
    db.execute(
        "UPDATE items SET current_stock = current_stock + 1 WHERE id = ?1",
        [item_id],
    )
    .ok();
    ...
    let mut stmt = db
        ...
        .unwrap();
    let batches: Vec<(i64, f64, f64)> = stmt
        .query_map(rusqlite::params![item_id, wh_id], |row| {
    ...
    let batch_refs: Vec<crate::calculations::stock::StockBatch> = batches
        .iter()
        .map(|(id, qty, cost)| crate::calculations::stock::StockBatch {
            id: *id,
    ...
    let mut stmt2 = db
        ...
        .unwrap();
    let batches2: Vec<(i64, f64, f64)> = stmt2
        .query_map(rusqlite::params![item_id, wh_id], |row| {
            Ok((row.get(0)?, row.get(1)?, row.get(2)?))
        })
        .unwrap()
    ...
    let batch_refs2: Vec<crate::calculations::stock::StockBatch> = batches2
        .iter()
        .map(|(id, qty, cost)| crate::calculations::stock::StockBatch {
            id: *id,
            quantity_remaining: *qty,
    ...
    db.execute(
        ...
        rusqlite::params![
            ...
            wh_id,
            1.0,
            fifo_result2.weighted_avg_cost,
            last_batch_id2
        ],
    )
    .ok();
    ...
    let remaining_batches: Vec<serde_json::Value> = query_report!(
        db,
        &format!(
            "SELECT batch_no, quantity_original, quantity_remaining, unit_cost
                  FROM stock_batches WHERE item_id = {} ORDER BY received_date, id",
            item_id
        )
    );
    ...
    db.execute("DELETE FROM stock_balances WHERE item_id = ?1", [item_id])
        .ok();
    db.execute("DELETE FROM items WHERE id = ?1", [item_id])
        .ok();
    db.execute("DELETE FROM warehouses WHERE id = ?1", [wh_id])
        .ok();

    let all_correct = trace
        .iter()
        .all(|t| t["correct"].as_bool().unwrap_or(false));
