    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(
                    json!({ "success": true, "data": { "id": inv_id, "invoice_no": invoice_no, "total_amount": total_amount } }),
                ),
            )
        }
        Err(e) => {
            ...
            (
                ...
                Json(json!({ "success": false, "error": "Failed to create POS sale." })),
            )
        }
    }
}
