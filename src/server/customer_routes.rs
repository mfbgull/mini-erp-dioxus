...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/customers/recalculate-balances",
            post(recalculate_balances),
        )
}
...
async fn get_customer(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            ...
            Json(json!({ "success": false, "error": "Customer not found." })),
        ),
    }
}
...
async fn create_customer(
    ...
    if form.customer_code.trim().is_empty() || form.customer_name.trim().is_empty() {
        return (
            ...
            Json(json!({ "success": false, "error": "Customer code and name are required." })),
        );
    }
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let exists: bool = db
        .query_row(
            "SELECT COUNT(*) > 0 FROM customers WHERE customer_code = ?1",
            [&form.customer_code],
            |row| row.get(0),
        )
        .unwrap_or(false);
    if exists {
        return (
            StatusCode::CONFLICT,
            Json(json!({ "success": false, "error": "Customer code already exists." })),
        );
    }
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                ...
                Json(json!({ "success": false, "error": "Failed to create customer." })),
            )
        }
    }
}
...
async fn update_customer(
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update customer." })),
            )
        }
    }
}
...
async fn delete_customer(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Customer deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Customer not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete customer." })),
            )
        }
    }
}
...
async fn customer_ledger(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let entries: Vec<CustomerLedgerEntry> = stmt
        .query_map([id], |row| {
...
async fn customer_statement(
    ...
    let entries: Vec<CustomerLedgerEntry> = stmt
        .query_map(rusqlite::params![id, start, end], |row| {
            Ok(CustomerLedgerEntry {
                id: row.get(0)?,
                customer_id: row.get(1)?,
                transaction_date: row.get(2)?,
...
async fn recalculate_balances(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let customer_ids: Vec<i64> = {
        let mut stmt = db
            .prepare("SELECT id FROM customers WHERE is_active = 1")
            .unwrap();
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": format!("Recalculated {} customers.", customer_ids.len()) } }),
        ),
    )
}
