...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            get(list_po_receipts).post(create_goods_receipt),
        )
        .route(
            "/api/purchase-orders/{id}/return-receipt",
            post(return_receipt),
        )
        .route("/api/purchase-orders/pending", get(list_pending_pos))
        .route(
            "/api/purchase-orders/summary/supplier/{id}",
            get(po_summary_by_supplier),
        )
        .route(
            "/api/purchase-orders/suppliers/{id}/balance",
            get(supplier_po_balance),
        )
        .route(
            "/api/purchase-orders/suppliers/{id}/transactions",
            get(supplier_po_transactions),
        )
        ...
        .route(
            "/api/purchase-orders/{id}/items/{item_id}",
            put(update_po_item).delete(delete_po_item),
        )
        // Direct Purchases
        .route(
            "/api/purchases",
            get(list_direct_purchases).post(create_direct_purchase),
        )
        .route(
            "/api/purchases/{id}",
            get(get_direct_purchase)
                .put(update_direct_purchase)
                .delete(delete_direct_purchase),
        )
...
async fn get_supplier(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(_) => (
            ...
            Json(json!({ "success": false, "error": "Supplier not found." })),
        ),
    }
}
...
async fn create_supplier(
    ...
    if form.supplier_code.trim().is_empty() || form.supplier_name.trim().is_empty() {
        return (
            ...
            Json(json!({ "success": false, "error": "Supplier code and name are required." })),
        );
    }
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let exists: bool = db
        .query_row(
            "SELECT COUNT(*) > 0 FROM suppliers WHERE supplier_code = ?1",
            [&form.supplier_code],
            |row| row.get(0),
        )
        .unwrap_or(false);
    if exists {
        return (
            StatusCode::CONFLICT,
            Json(json!({ "success": false, "error": "Supplier code already exists." })),
        );
    }
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                ...
                Json(json!({ "success": false, "error": "Failed to create supplier." })),
            )
        }
    }
}
...
async fn update_supplier(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Supplier updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Supplier not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update supplier." })),
            )
        }
    }
}
...
async fn delete_supplier(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Supplier deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Supplier not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete supplier." })),
            )
        }
    }
}
...
async fn next_supplier_code(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": { "next_code": code } })),
    )
}
...
async fn list_purchase_orders(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let result = db
        ...
        .and_then(|mut stmt| {
            stmt.query_map([], |row| {
                ...
                let (supplier_name, supplier_code): (Option<String>, Option<String>) = db
                    .query_row(
    ...
    match result {
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to list purchase orders." })),
            )
        }
    }
}
...
async fn get_purchase_order(
    ...
    let exists: bool = db
        .query_row(
            "SELECT COUNT(*) > 0 FROM purchase_orders WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .unwrap_or(false);
    if !exists {
        return (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Purchase order not found." })),
        );
    }
    ...
    match po {
        Ok(po) => {
            let items: Vec<PurchaseOrderItem> = db
                .prepare(
                ...
                .and_then(|mut stmt| {
                    ...
                    .map(|iter| {
                        iter.filter_map(|r| r.ok())
                            .collect::<Vec<PurchaseOrderItem>>()
                    })
                })
                .unwrap_or_default();
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "purchase_order": po, "items": items } })),
            )
        }
        Err(e) => {
...
async fn create_purchase_order(
    State(_state): State<AppState>,
    Json(form): Json<PurchaseOrderForm>,
) -> impl IntoResponse {
    ...
    let seq: i64 = db
        .query_row("SELECT COUNT(*) + 1 FROM purchase_orders", [], |row| {
            row.get(0)
        })
        .unwrap_or(1);
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": po })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to create PO: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create purchase order." })),
            )
        }
    }
}
...
async fn update_purchase_order(
    ...
    match result {
        Ok(rows) if rows > 0 => {
            db.execute("DELETE FROM purchase_order_items WHERE po_id = ?1", [id])
                .ok();
            for item in &form.items {
                let amount = item.quantity * item.unit_price;
                db.execute(
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Purchase order updated." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
        ...
        Err(e) => {
            tracing::error!("Failed to update PO: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update purchase order." })),
            )
        }
    }
}
...
async fn delete_purchase_order(
    ...
    match result {
        Ok(rows) if rows > 0 => {
            if let Err(e) = db.execute_batch("COMMIT") {
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        json!({ "success": false, "error": "Failed to delete PO (transaction rolled back)." }),
                    ),
                );
            }
            (
                StatusCode::OK,
                Json(
                    json!({ "success": true, "data": { "message": "Purchase order deleted and ledger reversed." } }),
                ),
            )
        }
        Ok(_) => {
        ...
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to delete PO: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete purchase order." })),
            )
        }
    }
}
...
async fn update_po_status(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<PurchaseOrderStatusUpdate>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Status updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Purchase order not found." })),
        ...
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update status." })),
            )
        }
    }
}
...
async fn list_pending_pos(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<PurchaseOrder> = stmt
        .query_map([], |row| {
            Ok(PurchaseOrder {
                id: row.get(0)?,
                po_no: row.get(1)?,
...
async fn list_po_receipts(
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<GoodsReceipt> = stmt
        .query_map([id], |row| {
            Ok(GoodsReceipt {
                id: row.get(0)?,
...
async fn create_goods_receipt(
    State(_state): State<AppState>,
    Path(po_id): Path<i64>,
    Json(form): Json<GoodsReceiptForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    if let Err(e) = db.execute_batch("BEGIN IMMEDIATE") {
        tracing::error!("Failed to begin transaction: {}", e);
    ...
    let seq: i64 = db
        .query_row("SELECT COUNT(*) + 1 FROM goods_receipts", [], |row| {
            row.get(0)
        })
        .unwrap_or(1);
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": gr_id, "receipt_no": rn } })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to create goods receipt: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create goods receipt." })),
            )
        }
    }
}
...
async fn return_receipt(
    ...
    // Get receipt details
    let receipt: Option<(i64, String, i64)> = db
        .query_row(
    ...
    let (receipt_id, receipt_no, warehouse_id) = match receipt {
        ...
        None => {
            return (
                StatusCode::NOT_FOUND,
                Json(json!({ "success": false, "error": "Receipt not found." })),
            )
        }
    };
    ...
    // Get items to return
    let mut stmt = db
        .prepare("SELECT item_id, received_quantity FROM goods_receipt_items WHERE receipt_id = ?1")
        .unwrap();
    let items_to_return: Vec<(i64, f64)> = stmt
        .query_map([receipt_id], |row| Ok((row.get(0)?, row.get(1)?)))
        .unwrap()
        .filter_map(|r| r.ok())
        .collect();
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": "Receipt return recorded.", "items_returned": items_to_return.len() } }),
        ),
    )
}
...
async fn list_direct_purchases(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<DirectPurchase> = stmt
        .query_map([], |row| {
            Ok(DirectPurchase {
                id: row.get(0)?,
                purchase_no: row.get(1)?,
                item_id: row.get(2)?,
                item_name: row.get(3)?,
                item_code: row.get(4)?,
...
async fn get_direct_purchase(
    ...
    match result {
        ...
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Purchase not found." })),
        ),
    }
}

async fn create_direct_purchase(
    State(_state): State<AppState>,
    Json(form): Json<DirectPurchaseForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    if let Err(e) = db.execute_batch("BEGIN IMMEDIATE") {
        tracing::error!("Failed to begin transaction: {}", e);
    ...
    let seq: i64 = db
        .query_row("SELECT COUNT(*) + 1 FROM purchases", [], |row| row.get(0))
        .unwrap_or(1);
    ...
    match result {
        Ok(_) => {
            ...
            if let Err(e) = db.execute(
                "UPDATE purchases SET batch_id = ?1 WHERE id = ?2",
                ...
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(json!({ "success": false, "error": "Failed to update purchase record." })),
                );
            }
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": purchase_id, "purchase_no": pno } })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to create direct purchase: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create purchase." })),
            )
        }
    }
}

async fn delete_direct_purchase(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    db.execute(
        "DELETE FROM stock_batches WHERE source_type = 'PURCHASE' AND source_id = ?1",
        [id],
    )
    .ok();
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Purchase deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Purchase not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to delete purchase: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete purchase." })),
            )
        }
    }
}

async fn update_direct_purchase(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
    Json(form): Json<DirectPurchaseForm>,
) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Purchase updated." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
        ...
        Err(e) => {
            tracing::error!("Failed to update purchase: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update purchase." })),
            )
        }
    }
}

async fn return_direct_purchase(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    let (purchase_id, quantity, item_id, warehouse_id, purchase_no) = match purchase {
        ...
        None => {
            return (
                StatusCode::NOT_FOUND,
                Json(
                    json!({ "success": false, "error": "Purchase not found or already returned." }),
                ),
            )
        }
    };
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": "Purchase return recorded.", "movement_no": mno } }),
        ),
    )
}
...
async fn po_summary_by_supplier(
    State(_state): State<AppState>,
    ...
    match result {
        ...
        Err(e) => {
            tracing::error!("Failed to get PO summary: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to get summary." })),
            )
        }
    }
}

async fn supplier_po_balance(
    State(_state): State<AppState>,
    Path(supplier_id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "data": { "balance": balance } })),
    )
}

async fn supplier_po_transactions(
    State(_state): State<AppState>,
    Path(supplier_id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
...
async fn add_po_item(
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": item_id, "amount": amount } })),
            )
        }
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to add item." })),
            )
        }
    }
}
...
async fn update_po_item(
    ...
    match result {
        Ok(rows) if rows > 0 => {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Item updated." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
...
async fn delete_po_item(
    ...
    let amount: f64 = db
        ...
        .unwrap_or(0.0);
    let result = db.execute(
        "DELETE FROM purchase_order_items WHERE id = ?1 AND po_id = ?2",
        rusqlite::params![item_id, po_id],
    );
    match result {
        Ok(rows) if rows > 0 => {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Item removed." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
...
async fn supplier_ledger(
    ...
    let entries: Vec<SupplierLedgerEntry> = stmt
        .query_map(rusqlite::params![supplier_id, from_date, to_date], |row| {
...
async fn create_supplier_payment(
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": id, "reference_no": ref_no } })),
            )
        }
        Err(e) => {
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to record payment." })),
            )
        }
    }
}
