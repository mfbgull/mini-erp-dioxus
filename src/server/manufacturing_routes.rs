...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/production/productions/summary/item/{id}",
            get(production_summary_by_item),
        )
}
...
async fn list_boms(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<Bom> = stmt
        .query_map([], |row| {
            Ok(Bom {
                id: row.get(0)?,
                bom_no: row.get(1)?,
                bom_name: row.get(2)?,
                finished_item_id: row.get(3)?,
                finished_item_name: row.get(4)?,
                finished_item_code: row.get(5)?,
                quantity: row.get(6)?,
                version: row.get(7)?,
...
async fn get_bom(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(bom) => {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "bom": bom, "items": items } })),
            )
        }
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "BOM not found." })),
        ),
    }
}
...
async fn create_bom(
    ...
    Json(form): Json<BomForm>,
) -> impl IntoResponse {
    if form.items.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "At least one raw material is required." })),
        );
    }
    if form.bom_name.trim().is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({ "success": false, "error": "BOM name is required." })),
        );
    }
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": bom_id, "bom_no": bom_no } })),
            )
        }
        Err(e) => {
            tracing::error!("Failed to create BOM: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create BOM." })),
            )
        }
    }
}
...
async fn update_bom(
    ...
    match result {
        Ok(rows) if rows > 0 => {
            db.execute("DELETE FROM bom_items WHERE bom_id = ?1", [id])
                .ok();
            for item in &form.items {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "BOM updated." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
        ...
        Err(e) => {
            tracing::error!("Failed to update BOM: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update BOM." })),
            )
        }
    }
}
...
async fn delete_bom(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "BOM deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "BOM not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to delete BOM: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete BOM." })),
            )
        }
    }
}

async fn toggle_bom_active(
    State(_state): State<AppState>,
    Path(id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "BOM status toggled." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "BOM not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to toggle BOM: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to toggle BOM." })),
            )
        }
    }
}

async fn get_bom_by_item(
    State(_state): State<AppState>,
    ...
    match result {
        Ok(bom) => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": bom })),
        ),
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "No active BOM found for this item." })),
        ),
    }
}
...
async fn list_productions(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let mut stmt = db
        ...
        .unwrap();
    let items: Vec<Production> = stmt
        .query_map([], |row| {
            Ok(Production {
                id: row.get(0)?,
                production_no: row.get(1)?,
                output_item_id: row.get(2)?,
                output_item_name: row.get(3)?,
                output_item_code: row.get(4)?,
                output_quantity: row.get(5)?,
                completed_qty: row.get(6)?,
                end_date: row.get(7)?,
                warehouse_id: row.get(8)?,
                warehouse_name: row.get(9)?,
                bom_id: row.get(10)?,
                bom_name: row.get(11)?,
                overhead_cost: row.get(12)?,
                batch_id: row.get(13)?,
                unit_cost: row.get(14)?,
                total_material_cost: row.get(15)?,
                status: row.get(16)?,
                notes: row.get(17)?,
    ...
    match result {
        Ok(prod) => {
            let mut stmt = db
                ...
                .unwrap();
            let inputs: Vec<ProductionInput> = stmt
                .query_map([id], |row| {
                    Ok(ProductionInput {
                        id: row.get(0)?,
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "production": prod, "inputs": inputs } })),
            )
        }
        Err(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Production not found." })),
        ),
    }
}
...
async fn create_production(
    ...
    let seq: i64 = db
        .query_row("SELECT COUNT(*) + 1 FROM productions", [], |row| row.get(0))
        .unwrap_or(1);
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({ "success": true, "data": { "id": prod_id, "production_no": pno } })),
            )
        }
        Err(e) => {
            let _ = db.execute_batch("ROLLBACK");
            tracing::error!("Failed to create production: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create production." })),
            )
        }
    }
}
...
async fn update_production(
    ...
    let result = db.execute(
        ...
        rusqlite::params![
            ...
            form.warehouse_id,
            form.bom_id,
            form.overhead_cost.unwrap_or(0.0),
    ...
    match result {
        Ok(rows) if rows > 0 => {
            ...
            db.execute(
                "DELETE FROM production_inputs WHERE production_id = ?1",
                [id],
            )
            .ok();
            for input in &form.inputs {
            ...
            (
                StatusCode::OK,
                Json(json!({ "success": true, "data": { "message": "Production updated." } })),
            )
        }
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(
                json!({ "success": false, "error": "Production not found or already completed." }),
            ),
        ),
        Err(e) => {
            tracing::error!("Failed to update production: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update production." })),
            )
        }
    }
}
...
async fn delete_production(
    ...
    if let Err(e) = db.execute_batch("COMMIT") {
        ...
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(
                json!({ "success": false, "error": "Failed to delete production (transaction rolled back)." }),
            ),
        );
    }
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "message": "Production deleted and stock reversed.", "inputs_restored": inputs.len() } }),
        ),
    )
}

async fn production_summary_by_item(
    State(_state): State<AppState>,
    Path(item_id): Path<i64>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let total: f64 = db
        .query_row(
            "SELECT COALESCE(SUM(output_quantity), 0) FROM productions WHERE output_item_id = ?1",
            [item_id],
            |row| row.get(0),
        )
        .unwrap_or(0.0);
    let count: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM productions WHERE output_item_id = ?1",
            [item_id],
            |row| row.get(0),
        )
        .unwrap_or(0);
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "total_produced": total, "production_count": count } }),
        ),
    )
}
