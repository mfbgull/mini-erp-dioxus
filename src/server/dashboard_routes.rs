...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/dashboard/stock-movement-summary",
            get(stock_movement_summary),
        )
...
async fn dashboard_summary(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let total_items: i64 = db
        .query_row("SELECT COUNT(*) FROM items WHERE is_active = 1", [], |r| {
            r.get(0)
        })
        .unwrap_or(0);
...
async fn sales_summary(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let today_count: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM invoices WHERE invoice_date = ?1 AND status != 'Cancelled'",
            [&today],
            |r| r.get(0),
        )
        .unwrap_or(0);
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "today": today_sales, "this_week": this_week, "this_month": this_month, "invoice_count_today": today_count } }),
        ),
    )
}
...
async fn expense_summary(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let by_category: Vec<serde_json::Value> = stmt
        .query_map([], |row| {
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "this_week": this_week, "this_month": this_month, "by_category": by_category } }),
        ),
    )
}
...
async fn production_status(State(_state): State<AppState>) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let in_progress: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM productions WHERE status = 'In Progress'",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0);
    let completed: i64 = db
        .query_row(
            "SELECT COUNT(*) FROM productions WHERE status = 'Completed'",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0);
    let total_output: f64 = db
        .query_row(
            "SELECT COALESCE(SUM(output_quantity), 0) FROM productions",
            [],
            |r| r.get(0),
        )
        .unwrap_or(0.0);
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "in_progress": in_progress, "completed": completed, "total_output_quantity": total_output } }),
        ),
    )
}
...
async fn kpi(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let low_stock: i64 = db
        ...
        .unwrap_or(0);
    let health = if total_items > 0 {
        ((total_items - low_stock) as f64 / total_items as f64) * 100.0
    } else {
        100.0
    };
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "stock_health": health, "total_items": total_items, "low_stock_count": low_stock } }),
        ),
    )
}
...
async fn ar_summary(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    (
        StatusCode::OK,
        Json(
            json!({ "success": true, "data": { "current": current, "days_1_30": d1_30, "days_31_60": d31_60, "days_61_90": d61_90, "days_90_plus": d90_plus } }),
        ),
    )
}
...
async fn list_layouts(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let items: Vec<DashboardLayout> = stmt
        .query_map([], |row| {
            Ok(DashboardLayout {
                id: row.get(0)?,
                user_id: row.get(1)?,
                layout_name: row.get(2)?,
                blocks: row.get(3)?,
...
async fn create_layout(
    ...
    Json(form): Json<DashboardLayoutForm>,
) -> impl IntoResponse {
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    ...
    match result {
        ...
        Err(e) => {
            tracing::error!("Failed to create layout: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to create layout." })),
            )
        }
    }
}
...
async fn update_layout(
    ...
    let db = db::get_db().lock().unwrap_or_else(|e| e.into_inner());
    let result = db.execute(
        "UPDATE dashboard_layouts SET layout_name=?1, blocks=?2 WHERE id=?3",
        rusqlite::params![form.layout_name, form.blocks, id],
    );
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Layout updated." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Layout not found." })),
        ),
        Err(e) => {
            tracing::error!("Failed to update layout: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to update layout." })),
            )
        }
    }
}
...
async fn delete_layout(State(_state): State<AppState>, Path(id): Path<i64>) -> impl IntoResponse {
    ...
    match result {
        Ok(rows) if rows > 0 => (
            StatusCode::OK,
            Json(json!({ "success": true, "data": { "message": "Layout deleted." } })),
        ),
        Ok(_) => (
            StatusCode::NOT_FOUND,
            Json(json!({ "success": false, "error": "Layout not found." })),
        ...
        Err(e) => {
            tracing::error!("Failed to delete layout: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({ "success": false, "error": "Failed to delete layout." })),
            )
        }
    }
}
