...
//! simulates async fetching of a single item by ID with its stock ledger.
...
/// mock stock ledger entries, and action buttons.
#[component]
pub fn ItemDetailPage(id: String) -> Element {
    ...
    let item_resource = use_resource(move || {
        ...
        async move {
            ...
            match client.get_item(parsed_id).await {
                Ok(server_item) => {
                    let status = derive_status(&server_item);
                    Some(InventoryItem {
                        id: server_item.id,
                        item_code: server_item.item_code,
                        item_name: server_item.item_name,
                        category: server_item.category,
                        unit_of_measure: server_item.unit_of_measure,
                        ...
                        status,
                        last_updated: server_item.updated_at,
                        warehouse: String::new(), // ponytail: not in detail endpoint
                    })
                },
                Err(_) => None,
            }
    ...
    // ── Pre-compute detail values ──
    let detail = item_opt.as_ref().map(|item| {
        let margin_pct = compute_margin(item);
        let stock_val = stock_value(item);
        let ledger: Vec<LedgerEntry> = Vec::new(); // ponytail: stock ledger endpoint not wired yet
        (item.clone(), margin_pct, stock_val, ledger)
    });

    if detail.is_none() {
        return rsx! { div {} };
    }
    let (ref item, margin_pct, stock_val, ref ledger) = detail.as_ref().unwrap();
    ...
    let on_edit = {
        ...
        move |_| {
            nav.push(format!("/inventory/items/{}", item_id));
            toast.info("Edit Mode", "Item editing coming soon — opening read-only detail.");
        }
    };
    ...
    rsx! {
        ...
        div { class: "page item-detail-page",
            ...
            // ════════ Detail View ════════
            else if let Some(ref detail_data) = detail {
                // ── Header ──
