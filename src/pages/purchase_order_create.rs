...
pub fn PurchaseOrderCreatePage() -> Element {
    ...
    let subtotal: f64 = items.read().iter().map(|li| li.quantity * li.rate).sum();
    let discount_amt: f64 = items.read().iter().map(|li| (li.quantity * li.rate) * (li.discount_pct / 100.0)).sum();
